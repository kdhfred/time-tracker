...
- 차트 표시
  - 최근 14일간의 카테고리별 사용 시간 표시
  - 스택형 막대 그래프 사용
    - X축: 날짜 (오늘/어제/MM월 DD일)
    - Y축: 시간 단위
  - 카테고리별 색상 구분
    - 골든 앵글(137.5도)을 이용한 색상 분배
    - 각 카테고리별 고유 색상 유지
  
- 차트 상호작용
  - 마우스 호버 시 상세 정보 표시
    - 카테고리명
    - 해당 날짜의 사용 시간 (시간/분 단위)
  - 하단 범례 클릭으로 카테고리 표시/숨김
  
- 다크모드 지원
  - 시스템/사용자 설정에 따른 차트 테마 자동 변경
  - 다크모드 시 가독성 최적화
    - 글자색 자동 조정
    - 그리드 라인 투명도 조정
    - 배경색 자동 변경
...
- CSV 내보내기
  - 모든 세션 기록을 CSV 파일로 내보내기
  - 파일명에 날짜 포함
  - UTF-8 인코딩 사용
  - 시작 시간, 종료 시간, 카테고리, 소요 시간 포함
